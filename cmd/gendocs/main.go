// Command gendocs generates man pages and shell completion scripts for pvmt.
...
func run() error {
	...
	// Bash
	bashFile, err := os.Create(filepath.Join(compDir, "pvmt.bash"))
	if err != nil {
		return err
	}
	...
	// Zsh
	zshFile, err := os.Create(filepath.Join(compDir, "_pvmt"))
	if err != nil {
		return err
	}
	defer zshFile.Close()
	...
	// Fish
	fishFile, err := os.Create(filepath.Join(compDir, "pvmt.fish"))
	if err != nil {
		return err
	}
